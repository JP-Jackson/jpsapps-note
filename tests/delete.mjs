...
const id = await mkEntry("typo entry to remove");
...
await p.click('#dayList .entry:has-text("typo entry")'); await p.waitForSelector("#deleteEntry", { timeout: 4000 });
...
const orphan = await p.evaluate(async () => (await (await fetch("/api/entries")).json()).entries.length);
check(orphan === 0, `and out of the log (${orphan} left)`);
...
{
  const id2 = await mkEntry("entry whose photo must really go");
...
const old = await mkEntry("older than the window", 20 * 60 * 1000);
...
await p.click('#dayList .entry:has-text("older than the window")'); await p.waitForTimeout(900);
...
const sub = await p.evaluate(async () => {
  const r = await fetch("/api/subjects", { method: "POST", headers: { "content-type": "application/json" },
    body: JSON.stringify({ name: "Scrap Compressor", type: "equipment", context: "work" }) });
  return (await r.json()).id;
});
const kept = await mkEntry("note that must outlive the thing");
...
await p.click('nav button[data-view="subjects"]'); await p.waitForTimeout(600);
await p.click('[data-subject-id]'); await p.waitForSelector("#editSubject", { timeout: 4000 });
...
await p.fill("#body", "queued mistake");
