...
await p.fill("#body", "compressor tripped again, breaker warm");
await p.click("#save");
await p.waitForSelector("#linkBar:not([hidden])", { timeout: 4000 });
const first = await p.$eval("#linkChips .chip", (n) => n.textContent.trim());
check(first === "Shop Compressor", `offline: "compressor" ranks Shop Compressor first (got "${first}")`);
...
const compEntries = await p.evaluate(async () => {
...
await p.fill("#body", "swapped the contactor on it");
await p.click("#save");
await p.waitForSelector("#linkBar:not([hidden])", { timeout: 4000 });
